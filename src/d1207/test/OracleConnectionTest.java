...
public class OracleConnectionTest {
	...
	//									ㄴ new 연산 외에 객체를 생성하는 다른 방법. 	: 메소드를 실행하여 할 수 있는 디자인 패턴	
			conn = DriverManager.getConnection(url,user,password);    //throws SQLExecption
		...
		} finally {		//최종적으로 자원(리소스)를 해제할 때 finally 안에 작성
			//db 접속 후 모든 db 작업을 종료하면 접속도 종료
			try {
				conn.close();
			} catch (SQLException e) {
			}
